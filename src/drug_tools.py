...
Adapted from workshop/src/tools.py for the Drug Interactions showcase.
Uses src.drug_rag instead of workshop.src.rag.
"""
...
class ToolKit:
    ...
    def search_drug_kb(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        ...
        results = drug_similarity_search(self._conn, query_embedding, k=top_k)
        return results
    ...
    def lookup_interactions(self, drug_name: str) -> list[dict[str, Any]]:
        ...
        results = drug_similarity_search(
            self._conn,
            query_embedding,
            k=5,
            filters={
    ...
    def summarize_evidence(self, findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        prompt = (
            ...
            "  observation whose metadata matches source_id.\n\n"
